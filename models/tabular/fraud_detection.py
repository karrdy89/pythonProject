...
# # df = pd.concat(map(pd.read_csv, [ROOT_DIR + "/dataset/fd_test2/1.csv",
# #                                  ROOT_DIR + "/dataset/fd_test2/2.csv",
# #                                  ROOT_DIR + "/dataset/fd_test2/3.csv",
# #                                  ROOT_DIR + "/dataset/fd_test2/4.csv",
# #                                  ROOT_DIR + "/dataset/fd_test2/5.csv"]), ignore_index=True)
# # seq = []
# # b_seq = 0
# # fc = 0
# # for index, row in df.iterrows():
# #     seq.append(row["SEQ"] + fc*2000)
# #     if b_seq > row['SEQ']:
# #         fc += 1
# #     b_seq = row['SEQ']
# #     # print(index, row['SEQ'])
...
import pandas as pd
from statics import ROOT_DIR
dataset_path = ROOT_DIR + "/dataset/fd_test2/combined_refine.csv"
df = pd.read_csv(dataset_path)
# add is normal
df["정상여부"] = ["정상"]*len(df)
...
df.drop(['메뉴', '메뉴명', "고객ID", "로그인IP", "프로그램명", "금액", "로그인유형", "비고", "DATE"], axis=1, inplace=True)
...
tabular_dataset.to_csv(ROOT_DIR + "/dataset/fd_test2/combined_tabular.csv", sep=",",
